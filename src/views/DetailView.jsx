...
const ACTION_LABELS = {
  ...
  CANCELLED: 'Cancelar',
  PAYMENT_INITIATED: 'Marcar iniciado',
  ACTIVE: 'Reativar',
};
const ACTION_STYLES = {
...
export default function DetailView() {
  ...
  if (problem) return (
    <div className="max-w-xl">
      ...
      <Link to="/" className="inline-block mt-4 text-sm font-semibold text-petrol-600">← Voltar à lista</Link>
    </div>
  );
    ...
    <div>
      {state?.justCreated && (
        <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
          ...
          QR code criado — pronto para ser apresentado ao pagador.
        </motion.div>
      )}
      ...
      <div className="mt-8 grid lg:grid-cols-[380px_1fr] gap-6 items-start">
        ...
        <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}
          ...
          <div className="mt-3 flex gap-4">
            <button onClick={() => downloadQR(emv, `qr-${qr.id}`, 'png')} className="text-xs font-bold text-petrol-600 hover:text-petrol-800">Baixar PNG</button>
            <button onClick={() => downloadQR(emv, `qr-${qr.id}`, 'svg')} className="text-xs font-bold text-petrol-600 hover:text-petrol-800">Baixar SVG</button>
            <CopyButton text={emv} label="Copiar EMV" />
          </div>
          <div className="mt-5 w-full">
            <div className="text-[11px] font-bold uppercase tracking-[0.12em] text-ink/40 mb-1.5">String EMV</div>
        ...
        <div className="space-y-5">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <Field label="Valor">
                <span className="font-display text-2xl">
                  {qr.bill?.amountDue ? formatAmount(qr.bill.amountDue.amount, qr.bill.amountDue.currency) : 'valor livre (editável)'}
                </span>
              </Field>
              <Field label="Descrição">{qr.bill?.description || '—'}</Field>
              <Field label="Timing">{qr.bill?.paymentTiming}</Field>
              <Field label="Válido até">{new Date(qr.validUntil).toLocaleString('pt-BR')}</Field>
              {qr.bill?.tip && (
                <Field label="Gorjeta" className="sm:col-span-2">
                  {qr.bill.tip.presets?.map((p) => `${(p / 10).toFixed(1)}%`).join(' · ')}
                  {qr.bill.tip.range && ` (faixa ${(qr.bill.tip.range.min / 10).toFixed(1)}–${(qr.bill.tip.range.max / 10).toFixed(1)}%)`}
                </Field>
              )}
            </div>
          </div>
              ...
              {(qr.paymentMethods || []).map((m, i) => (
                <div key={i} className="rounded-xl border border-ink/10 p-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs text-ink/40">até {new Date(m.validUntil).toLocaleDateString('pt-BR')}</span>
                  </div>
                  <div className="mt-2 space-y-1.5">
                    {Object.entries(m.networks || {}).map(([net, cfg]) => (
                      <div key={net} className="flex items-center gap-2 text-xs font-mono text-ink/60">
                        ...
                        {cfg.routingNumber && <span>ABA {cfg.routingNumber} · conta {cfg.accountNumber} · {cfg.protectionType}</span>}
          ...
          <div className="card p-5">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <Field label="Notificação">{typeof qr.paymentNotification === 'string' ? qr.paymentNotification : qr.paymentNotification?.kind}</Field>
        ...
        {payModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95, y: 10 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0 }}
              ...
              <h3 className="font-display text-2xl">Marcar como pago</h3>
              <p className="text-sm text-ink/55 mt-1">O contrato exige a rede e o <span className="font-mono text-xs">endToEndId</span> da transação.</p>
              <div className="mt-5 space-y-4">
                <label className="block">
                  <span className="text-[11px] font-bold uppercase tracking-[0.12em] text-ink/40">Rede</span>
                ...
                <label className="block">
                  ...
                  <input value={payForm.endToEndId} onChange={(e) => setPayForm({ ...payForm, endToEndId: e.target.value })}
                    placeholder="ex.: 0196cb82-afab-46f1-bf8d-fff2f9c8a53e"
              ...
              <div className="mt-6 flex justify-end gap-3">
                <button onClick={() => setPayModal(false)} className="px-4 py-2 rounded-xl text-sm font-bold text-ink/50 hover:bg-ink/5">Voltar</button>
